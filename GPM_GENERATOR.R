...
GPM$generar_urls <- function(fecha_inicial, fecha_final){
  ...
  # URLs de las páginas
  urlbase <- "https://gpm1.gesdisc.eosdis.nasa.gov/data/GPM_L3/GPM_3IMERGDE.07/"
  ...
  for (fecha in as.character(rango)) {
    ...
    filename <- paste0("3B-DAY-E.MS.MRG.3IMERG.",date_curr ,"-S000000-E235959.V07B.nc4")
...
GPM$downloads_transformar_a_tif <- function(Rango_fecha, lugar, parametro) {
  ...
  tryCatch({    
    ...
    url_base <- '/opt/shiny-server/samples/sample-apps/datosgrillados/GPM'
            ...
            tryCatch(
              {
                ...
                ruta_archivo <- paste0("3B-DAY-E.MS.MRG.3IMERG.",fecha,"-S000000-E235959.V07B.nc4")
                ...
                # Lee la variable precipitation
                variable_precipitacion <- ncvar_get(archivo_nc, "precipitation")
