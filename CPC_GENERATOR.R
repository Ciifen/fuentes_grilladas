...
CPC$generate_raster_info_txt <- function(raster_path) {
  ...
  # Convertir la lista de informaci?n a un formato legible
  base_text <- paste(
  ...
  # Crear la ruta para el archivo de texto con el mismo nombre pero extensi?n .txt
  ...
  # A?adir la informaci?n de las bandas y las fechas
...
CPC$crear_subdirectorios <- function(base, ...) {
  ...
  if (!dir.exists(ruta)) {
    ...
    print(paste("La carpeta", ruta, "no exist?a y fue creada."))
  } else {
...
CPC$downloads_transformar_a_tif <- function(Rango_fecha, lugar, parametro) {
  ...
  tryCatch({   
    ...
    # Definir el n?mero de n?cleos a utilizar
    #num_cores <- detectCores()-4
    #num_cores <- 2
    
    # Registrar el backend paralelo con el n?mero de n?cleos especificado
    ...
    url_base <- '/opt/shiny-server/samples/sample-apps/datosgrillados/CPC'
      ...
      tryCatch({
        ...
        for (i in seq_along(urls)) {
          ...
          if (file.exists(file_temp)) {
            if (as.Date(file.info(file_temp)$ctime) != as.POSIXlt(Sys.Date(), tz = "UTC")){
        ...
        for (i in seq_along(urls_tif_filename)) {
          ...
          if (!existe(url_tif_pais)) {
            ...
            # El archivo .tif no existe, desc?rgalo y procesa
            tryCatch(
              {
                ...
                # Definir la extensi?n (ajusta las coordenadas seg?n la necesidad)
              ...
              error = function(cond) {
                # Si se produce una excepci?n, se maneja aqu?
        ...
        # Crear el nombre del archivo zip con la misma basename pero extensi?n .zip
